...
public class ExceptionUtil {
	 ...
	 */
	static public RuntimeException throwUnchecked(Throwable t) {
		if (t == null ) {
	 ...
	 */
	static public <EX extends Throwable> EX throwDeepCauseChecked(Throwable t, Class<? extends EX> clazz) throws EX {
		if (t == null ) {
		   throw new NullPointerException();
	    }
	    ...
	    EX rslt = null;
	    try {
	    	rslt = (EX)clazz.getConstructor(Throwable.class).newInstance(t);
		}
		catch (Exception e) {
			try {
				rslt = (EX)clazz.newInstance();
				rslt.initCause(t);
			}
			catch (Exception e1) {
				if (e1.getCause()==null) {
