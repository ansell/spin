...
public class JenaUtil {
	...
	private final static String WITH_IMPORTS_PREFIX = "http://rdfex.org/withImports?uri=";
	...
	private static void addTransitiveObjects(Set<Resource> resources, Resource subject, Property predicate) {
		addTransitiveObjects(resources, new HashSet<Resource>(), subject, predicate);
	}
		...
		if (object != null) {
			...
			while (it.hasNext()) {
				...
				if (!reached.contains(subject)) {
					addTransitiveSubjects(reached, (Resource)subject, predicate);
				}
			}
		}
	}
	...
	private static Set<Resource> getAllTransitiveObjects(Resource subject, Property predicate) {
	...
	public static List<Statement> getStatementsList(StmtIterator it) {
		List<Statement> results = new LinkedList<Statement>();
		while(it.hasNext()) {
			results.add(it.nextStatement());
		}
		return results;
	}
	 ...
	 * a subclass thereof.  Make sure that the type parameter is associated
	 * with the right Model, because the system will try to walk down into
	 * the subclasses of type.
	 * @param instance  the Resource to test
	 * @param type  the type
	 * @return true if resource has type type
	 */
	public static boolean hasIndirectType(Resource instance, Resource type) {
		if (instance.hasProperty(RDF.type, type)) {
			return true;
		}
		for(Resource subType : getAllSubClasses(type)) {
			if (instance.hasProperty(RDF.type, subType)) {
				return true;
			}
		}
		return false;
	...
	public static boolean hasSuperClass(Resource subClass, Resource superClass) {
		return getAllSuperClasses(subClass).contains(superClass);
	}
	...
	public static boolean isMemoryGraph(Graph graph) {
		if(graph instanceof MultiUnion) {
			for(Graph subGraph : getSubGraphs((MultiUnion)graph)) {
		...
		else if(graph instanceof GraphMemBase) {
			return true;
		}
		else {
			return false;
